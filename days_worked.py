        ...
        for index, row in df.iterrows():
            ...
            for line in lines:
                ...
                if author in authorList:
                    ...
                    newFilesList.append(daysList[authorList.index(author)])
                else:
                    for authorItem in duplicates.keys():
                        if author in duplicates[authorItem]:
                            newAuthorsList.append(author)
                            newFilesList.append(daysList[authorList.index(authorItem)])
                            break
            filesFile = open(targetPath + repoName + '/daysWorked.txt', "w", encoding='utf-8', errors='ignore')
            i = 0
            for line in lines:
                author = line.split('\t')[1].strip()
